...
class uint8:
    @staticmethod
    def make(generics):
        if isinstance(generics, uint8):
            return generics
        else:
            return uint8(generics)

    def __init__(self, generics):
        if isinstance(generics, int):
            self.value = generics & 0xFF
        elif isinstance(generics, bytes):
            self.value = int.from_bytes(generics, 'little') & 0xFF
        elif isinstance(generics, str):
            self.value = int.from_bytes(bytes(generics, 'ascii'), 'little') & 0xFF
        elif isinstance(generics, collections.Iterable):
            self.value = int.from_bytes(bytes(generics), 'little') & 0xFF
        else:
            raise TypeError

    def __add__(self, other):
        other = uint8.make(other)
    ...
    def __getitem__(self, index):
        return self.value.to_bytes(1, 'little')[index]
