...
class _function:
    ...
    def define(new, old, mark):
        def wrapper(*args, **kwargs):
            ...
            if not depth(equal=lambda f1, f2: f1.f_locals['mark'] == f2.f_locals['mark']):
                ...
                with frames(filter=lambda f: f.f_code is __class__.invoke.__code__) as f:
                    assert f.has(0)
                    d = f[0].f_locals['d']
                assert d
                return d[0]
    ...
    @staticmethod
    def invoke(*d):
        with frames(filter=lambda f: f.f_code is __class__.define(None, None, None).__code__) as f:
            assert f.has(0)
